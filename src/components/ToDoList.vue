      ...
      <div class='col post'>
        ...
        <div id='app'>
          <div class='input-group mb-3'>
            ...
            <input type='text' class='form-control' placeholder='準備要做的任務' v-model="newData">
            <div class='input-group-append'>
              <button class='btn btn-primary text-light' type='button' @click="addList()">新增</button>
            </div>
          </div>
          <div class='card text-center'>
            <div class='card-header'>
              <ul class='nav nav-tabs card-header-tabs'>
                <li class='nav-item' v-for="item in option" :key="item.name" :v-bind= item @click="switchOption(item)" >
                  <a class='nav-link' href='#' :class="{ active: item.isActive }"
                  >{{ item.name }}</a>
                </li>
              </ul>
            </div>
            <ul class='list-group list-group-flush text-left'>
              <li class='list-group-item' v-for="item in filterList" :key="item.name">
                <div class='d-flex'>
                  ...
                  <button type='button' class='close ml-auto' aria-label='Close' @click="deleteList( item.name )">
            ...
            <div class='card-footer d-flex justify-content-between'>
              <span>還有 {{ todoCount }} 筆任務未完成</span>
              <a href='#' @click="clearList()">清除所有任務</a>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    switchOption (e) {
      this.option.forEach((item, index, array) => {
        item.isActive = false
      })
      e.isActive = true
      filterList(this)
...
.btn-primary {
  ...
  .btn-primary:focus, .btn-primary.focus {
    box-shadow: 0 0 0 0.2rem rgba(79, 192, 141, 0.5); }
...
.custom-control-input {
  ...
  .custom-control-input:focus ~ .custom-control-label::before {
    box-shadow: 0 0 0 1px #fff, 0 0 0 0.2rem rgba(79, 192, 141, 0.25); }
...
.custom-select {
  ...
  .custom-select:focus {
    border-color: #ade2ca;
    outline: 0;
    box-shadow: inset 0 1px 2px rgba(0, 0, 0, 0.075), 0 0 5px rgba(173, 226, 202, 0.5); }
...
button.bg-primary:focus {
  background-color: #3aa273 !important; }
...
.bg-primary-gradient {
  background: linear-gradient(90deg, #3292a6 15%, #B97EE4 75%, #BA8AFF 94%); }
